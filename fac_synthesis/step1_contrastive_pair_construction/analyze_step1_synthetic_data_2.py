...
def analyze_data():
    ...
    # This is the "global_best" used as the Good example in the contrastive pair.
    ...
    print(f"Info: Computed global_best for {len(global_best_by_feature)} features.")
        ...
        for fid in ordered_feature_ids:
            ...
            if samples:
                ...
                outj.write(json.dumps({
                    'feature_id': fid,
                    'samples': samples,
                    'global_best': global_best_by_feature.get(int(fid))
                }, ensure_ascii=False) + '\n')
